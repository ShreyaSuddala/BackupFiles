...
export default function DataGridDemo11() {
  const columns = [
    { 
        field: 'id', 
         headerName: 'ID',
  ...
  const [editedEmpId, setEditedEmpId] = useState('');
  const [editedMonth, setEditedMonth] = useState('');
  const [editedYear, setEditedYear] = useState('');
  const [editedTechnology, setEditedTechnology] = useState('');
  const [editedTechnicalRating, setEditedTechnicalRating] = useState('');
  const [editedCommunicationRating, setEditedCommunicationRating] = useState('');
  const [editedRemarks, setEditedRemarks] = useState('');
  const [editedReviewedBy, setEditedReviewedBy] = useState('');
  ...
  const handleEdit = (rowId) => {
    ...
    setEditedMonth(rowToEdit.empId);
    setEditedMonth(rowToEdit.month);
    setEditedYear(rowToEdit.year);
    setEditedTechnology(rowToEdit.technology);
    setEditedTechnicalRating(rowToEdit.technicalRating);
    setEditedCommunicationRating(rowToEdit.communicationRating);
    setEditedRemarks(rowToEdit.remarks);
    setEditedReviewedBy(rowToEdit.reviewedBy);
  ...
  const handleSaveChanges = () => {
    ...
    const updatedRow = {
      ...
      technicalRating: editedTechnicalRating,
      communicationRating: editedCommunicationRating,
      remarks: editedRemarks,
      reviewedBy: editedReviewedBy,
    };

    saveDataToServer(updatedRow)
      .then(() => {
          ...
          <Dialog open={openDialog} onClose={handleCloseDialog}>
            ...
            <DialogContent>
              ...
              <TextField
                ...
                onChange={(e) => setEditedEmpId(e.target.value)}
              />
